...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0, voted = 0; j < candidate_count && voted != 1; j++)
        {
...
bool is_tie(int min)
{
    int number_of_candidates_still_in_the_election = 0;
    int number_of_same_candidates_with_min_votes = 0;
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].eliminated == false)
        {
            number_of_candidates_still_in_the_election++;
            if (min == candidates[i].votes)
            {
                number_of_same_candidates_with_min_votes++;
            }
        }
    }
